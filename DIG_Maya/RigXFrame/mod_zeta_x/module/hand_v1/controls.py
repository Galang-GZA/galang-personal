...
from galang_utils.rig_x_frame.mod_zero.hand.guide import Hand_GuideInfo


class Hand_ControlCreator:
    def __init__(self, guide, kinematics, node_level=NODE_MAIN_LEVELS):
        ...
        self.node_flags = {level: NODE_DEFAULT_FLAGS.get(level, False) for level in self.node_level}

    def get_node(self, level):
        # Returns the transform node for the given node level (e.g., 'offset', 'sdk')
        return self.nodes.get(level)

    def create(self, color_set=MAIN_COLOR):
        NK = None
