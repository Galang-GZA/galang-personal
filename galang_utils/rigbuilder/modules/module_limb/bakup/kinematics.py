...
class Limb_FKSetup:
    def __init__(self, guide):
        ...
        self.fk_joint_chain = Limb_JointChainSetup(guide, FK)
    ...
    def build(self):
        ...
        for g in self.input.guides:
            if g.is_guide_end or g.is_guide_misc:
                continue
            fk_control = Limb_ControlCreator(g.name, FK)
            fk_control.create()
...
class Limb_IKSetup:
    def __init__(self, guide):
        self.guide = Limb_GuideInfo(guide)
        self.input = Limb_GuideList(guide)
        self.ik_joint_chain = Limb_JointChainSetup(guide, IK)
    ...
    def build(self):
        ...
        for g in self.input.guides:
            ...
            if g.module == LIMB:
                start_ik_joint = jnt
                start_ik_control = Limb_ControlCreator(g.name, IK)
                ...
                start_ik_control_local = Limb_ControlCreator(g.name, IK)
                ...
                for c in hinges_guide_children:
                    c_info = Limb_GuideInfo(c)
                    if c_info.is_guide_misc:
                        pv_guide_joint = c
                pole_vector_control = Limb_ControlCreator(pv_guide_joint, IK)
                ...
                end_ik_joint = jnt
                end_ik_control = Limb_ControlCreator(g.name, IK)
        ...
        ik_handle_name = limb_level_format(PJ, IK, self.guide.side, limb_name, level=None)
        ik_solver_name = limb_level_format(PJ, IK, self.guide.side, limb_name, level=None, item="RPsolver")
