...
class LimbSettingComponent:
    def __init__(self, guide):
        ...
        self.limb_setting = None

    def create(self):
        guide_setting = self.module.guides[0]
        self.limb_setting = LimbControlCreator(guide_setting.name, MODULE)
        self.limb_setting.create()

        #   Lock and hide oiriginal attributes
        attrs = ["tx", "ty", "tz", "rx", "ry", "rz", "sx", "sy", "sz", "visibility"]
        for attr in attrs:
            cmds.setAttr(f"{self.limb_setting.ctrl}.{attr}", lock=True, keyable=False, channelBox=False)

        #   Create kinematic switch attribute
        if not cmds.attributeQuery(IKFKSWITCH, node=self.limb_setting.ctrl, exists=True):
            cmds.addAttr(self.limb_setting.ctrl, ln=IKFKSWITCH, at="double", min=0, max=1, dv=0, keyable=True)
