    ...
    def create(self, color_set: Dict = task_setup.MAIN_COLOR, node_level=task_setup.NODE_MAIN_LEVELS, local=None):
        ...
        if shape_data:
            self.ctrl = cmds.curve(
                ...
                name=self.format.name(self.guide.name_raw, P_ROLE.CTRL),
            )
            ...
            self.ctrl = cmds.circle(
                n=self.format.name(self.guide.name_raw, P_ROLE.CTRL),
        ...
        for level in node_level:
            ...
            if self.node_flags[level]:
                node = cmds.group(
                    em=True,
                    name=self.format.name(self.guide.name_raw, P_ROLE.CTRL, level=level),
