...
class LimbFKComponent:
    ...
    def create(self):
        ...
        fk_grp_types = [TASK_ROLE.TOP]
        fk_grp = LimbGroupCreator(fk_grp_types, self.module)
        fk_grp.create()
        self.groups = fk_grp.map

        fk_grp_top = self.groups.get(TASK_ROLE.TOP)

        # step 1: Create FK joint chain
        fk_jnt = LimbJointChainSetup(self.guide.name, TASK_ROLE.FK)
        fk_jnt.create()
        self.groups[TASK_ROLE.JNT] = fk_jnt.group
        ...
        for guide in self.module.guides:
            ...
            fk_manipulator = LimbControlCreator(guide, TASK_ROLE.FK, self.module)
