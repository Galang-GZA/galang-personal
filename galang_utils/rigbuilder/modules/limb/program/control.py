...
class LimbControlCreator:
    def __init__(self, guide: GuideInfo, kinematics, module: ModuleInfo):
        ...
        self.top = None
        self.nodes:Dict = {}
        self.limb_format = LimbFormat(self.kinematics, self.guide.side)
    ...
    def create(self, color_set: Dict = task_setup.MAIN_COLOR, node_level=task_setup.NODE_MAIN_LEVELS, local=None):
        ...
        if P_ROLE.PV in self.guide.name:
        ...
        for level in node_level:
            ...
            if self.node_flags[level]:
                node = cmds.group(
                    em=True,
                    name=self.limb_format.name(self.guide.name_raw, P_ROLE.CTRL, level=level),
                    
                )
                if self.guide.side_id == task_setup.MIRROR_SIDE_ID and level == P_ROLE.MIRROR:
