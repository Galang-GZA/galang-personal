    ...
    def create(self, color_set=MAIN_COLOR):
        ...
        if shape_data:
            self.ctrl = cmds.curve(
                ...
                name=control_format(self.kinematics, self.guide.side, self.guide.name_raw),
            )
            ...
            self.ctrl = cmds.circle(
                n=control_format(self.kinematics, self.guide.side, self.guide.name_raw),
        ...
        for level in NODE_LEVELS:
            if self.node_flags[level]:
                node = cmds.group(
                    em=True, name=level_format(self.kinematics, self.guide.side, self.guide.name_raw, level, item=CTRL)
                )
